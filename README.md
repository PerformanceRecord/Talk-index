...
### 軽量レコメンド/検索の仕組み（2026-04 更新）

- `latest.json` は**初期表示に必要な最小データ**だけを持ちます。
- `search_index.json` は exporter 側で事前生成した `token -> エントリID一覧` を持ちます。
- ブラウザは初回ロード時に index を作りません。
- 検索入力にフォーカスした時だけ `search_index.json` を lazy-load します（1回のみ）。
- 読み込み失敗時もページ表示と通常検索は継続します。
...
索引シートをJSON化し、Cloudflare R2 の `index/latest.json` と `index/search_index.json` に配置します。
...
- `index/latest.json`
  - `version`
  - `generated_at`
  - `items`（初期表示用の最小フィールド: `id/title/date/url/section/section_url/subsection/tags`）
