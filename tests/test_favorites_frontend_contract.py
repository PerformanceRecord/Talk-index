...
class FavoritesFrontendContractTests(unittest.TestCase):
    ...
    def test_video_and_talk_views_have_cross_navigation(self):
        ...
        self.assertNotIn('収録トークを見る', app_js)
        self.assertIn('の収録トークを${openKeys.has(video.key) ? "閉じる" : "開く"}', app_js)
        self.assertIn('metaRow.textContent = `収録トーク ${video.sectionCount || 0}件`;', app_js)
